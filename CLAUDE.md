...
**The drive screen has one z-order, `DRIVE_LAYER` in `driveLayers.ts`**, and it spans two files: controls come from `GameCanvas`, the HUD from `SideSwapApp`, and they are siblings in `.game-page`'s stacking context. Hard-coding a z-index in either is how the steering control ended up painted under the wallet card and the pedals under the minimap for months — invisible, yet still tappable through the HUD's `pointerEvents: "none"`, so nothing failed and no test could see it. `shellStyle` must therefore **not** set `isolation: "isolate"`: that makes the GameCanvas subtree atomic at its own level and no control inside can outrank a HUD sibling.

Touch controls live in `TouchDriveControls.tsx` with **no Babylon import**, so `tests/touchDriveControls.test.tsx` can render them in jsdom. Steering is drag-with-a-floating-origin (`touchSteering.ts`, pure); the release ease runs in `fixedUpdate` via `touchSteerReleasing`, never in React. `TOUCH_TOP_RAIL_PX` is shared with `SideSwapApp` because a landscape phone is ~393px tall and the right edge has to fit buttons, then minimap, then pedals, in that order.
