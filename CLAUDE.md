...
- **`env(safe-area-inset-*)` is `0px` without `viewport-fit=cover`, and `vinext`'s `Viewport` shim cannot emit it** (unlike Next's, it has no `viewportFit` field), so `applyViewportFitCover` patches the rendered tag at runtime from `SideSwapApp`'s hydration effect. Remove that call and every notch inset in the HUD and controls silently becomes zero.
- **`screen.orientation.lock()` has never shipped in Safari**, so landscape cannot be enforced on iPhone — the rotate gate is the only lever, and it must stay cheap: it pauses the session, and must never again gate session *construction*, which made every rotation rebuild the whole city. `requestLandscapeLock` is an Android-only nicety and runs inside the click, under the same transient-activation rule as `primeAudioContext`.
