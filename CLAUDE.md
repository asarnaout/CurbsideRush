...
The game pivoted from a driving curriculum to an open-world gig driver: the lessons were deleted and the save key is `sideswap:v2` (`app/game/progress.ts:16`). The README was rewritten for the current game and no longer mentions lessons or `sideswap:v1` — it is the player-facing picture, this file is the internals.

**`lesson` survives as internal vocabulary.** `GameCanvasLesson` (`GameCanvas.tsx:1044`) is the *runtime scenario contract*, and the only scenario type left is free drive: `buildFreeDriveLesson` / `buildCareerDayLesson` (`app/game/freeDriveLesson.ts`) both produce `kind: "free_drive"` with empty `route`/`checkpoints`/`coachPrompts`, and `SideSwapApp` calls one of them every render (`SideSwapApp.tsx:897-904`). Vestigial dead branches remain for retired content: `roadIdForLane` still maps `yard-*`/`xf-*` ids, `RoadSurfaceType` still has `"orientation"`, `GameCanvas` defaults `mapId` to `"orientation-yard"`. No such map exists.
...
| Derived | From | Where |
|---|---|---|
| asphalt strips, kerb/junction fills | `roadSurfaces` | `GameCanvas.tsx:532`, `:800` |
...
| instanced building street wall | `blocks.buildingSet` | `buildingSets.ts:210` |
| signal phase clock | `controls.phaseGroup` | `trafficSignals.ts:54` |

`getMapPack(id)` (`content.ts:2068`) is a pure frozen lookup that throws on unknown ids.
...
- **Per-vehicle physics are `SimulationCoreConfig` fields whose defaults equal the old literals exactly** — the acceptance replay pins that identity, so never change a default without meaning to change free drive. NPC-to-NPC spacings are deliberately pinned literals (they must not re-space around the player's vehicle). Note the catalog's caps all sit *below* free drive's: `vehiclePhysics` is spread **after** the adapter's config (`GameCanvas.tsx:3435`), and the adapter sets 31.29 m/s (70 mph) while the reference hatch takes the core's own default of 22 — so every career vehicle, sports car included, is slower than the free-drive car.
- **The five-vehicle catalog splits three ways visually** (`career.ts` `CAREER_VEHICLES`): three car models, plus the bicycle and motorbike as composed rigs (`model: null`). Both two-wheelers force third-person — there is no cockpit to sit in (`GameCanvas.tsx:3405`) — take `buildBikeErrandScript` with their own `CutsceneBodyProfile` (`BIKE_`/`MOTORBIKE_CUTSCENE_BODY`) instead of car doors, and hide the rider on the vehicle for the scene's duration so the walking actor reads as the same person (`:4199`, `:4266`, `:4341`). Cars take a `CutsceneBodyProfile` scaled from `VEHICLE_DIMENSIONS`; the flagship reproduces the defaults exactly.
...
`GameCanvas.tsx` is ~11.3k lines but only two live objects: `class BabylonGameSession` (`:3196`) and the React component (`:10620`). **React owns the canvas element, the props, and one 10 Hz HUD snapshot; the session owns everything else.** No React state is driven at frame rate. The session is rebuilt only on `[trafficSide, steeringSide, lesson?.id, mapPack?.id, sessionActivation]`; every other prop flows through `session.updateOptions(...)`.

Everything above `GameCanvasProps` (`:1461`) is an **exported pure geometry layer** (road strips, junction fills, chevron placement) — exported specifically so tests can import them without instantiating Babylon.
...
`resolveMapVisualKey(mapId)` (`visuals.ts:150`) is **substring matching with an `nyc` default** — a typo'd or new map id silently gets NYC's night+paved palette, which changes lighting, fog, ground texture, sidewalk width *and* the crowd's rail geometry.
...
- **Under `"coach"` enforcement nothing hard-resets the player — including collisions.** `enforcement` is `"coach"` iff `lesson.kind === "free_drive"` (`simulationAdapter.ts:1314`), which is every free drive *and* every career day. `flagCritical` softens all four codes it is ever called with — `collision` against static geometry (`simulation.ts:2410`), `wrong_way` (`:3612`), `out_of_bounds` (`:3628`), `red_light` (`:3918`) — into non-terminating `minor` events; NPC-vehicle collisions are softened inline in `checkCollisions` (`:4010`, which separates the bodies, scrubs speed, knocks the NPC askew and `continue`s); pedestrian/cyclist/prop contact is softened in `reportExternalContact` (`:1913`). The one unconditional `triggerCritical` entry point left is `reportExternalCollision` (`:1886`), reachable only through `session.reset(incidentMessage)` (`GameCanvas.tsx:3754`) — and no caller passes that argument, so it is dead in practice. Wiring up `reset(message)` would silently resurrect checkpoint-teleporting in the open world.
- **A fine needs a witness, and covers collisions.** `GameCanvas.tsx:6288` emits `fine` only when `patrolNearPlayer(35)` finds a patrol, for `wrong_way`, `out_of_bounds`, `red_light`, **and** `collision` without an `evidence.roadUserType`. Hitting a person is the exception: the app cites that unconditionally, on its own 4s debounce (`SideSwapApp.tsx:829-851`), separate from the 8s debounce on witnessed fines.
