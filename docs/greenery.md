...
There are **sixteen** authored parks, not the fourteen you get by reading the
content files: London's two roundabout islands are generated by its
turning-loop helper rather than listed. `tests/parkLayouts.test.ts` pins the
total, eighteen, including those two.
